...
export const validateVocation = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (vocation.length < 3) {
    return res.status(422).json({ message: '"vocation" length must be at 3 characters long' });
  }
  next();
};
...
export const validationPassword = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (typeof password !== 'string') {
    return res.status(422).json({ message: '"password" must be at string' });
  }

  if (password.length < 8) {
    return res.status(422).json({ message: '"password" length must be at 8 characters long' });
  }
  next();
};